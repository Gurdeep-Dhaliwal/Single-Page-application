...
import { login, isAdmin } from 'accounts'
import { contentType } from 'mediaTypes'
import { showShows } from '/home/codio/workspace/api/modules/shows.js'
...
async function validCredentials(context, next) {
	...
	if(path === '/api/showShows' && method === 'GET') {
		try{
			const credentials = extractCredentials(token)
			console.log(credentials)
			const shows = await showShows(credentials)
			console.log(`MiddleWare: `,shows)
		} catch(err) {
			console.log('ERROR')
			console.log(err)
			console.log(`msg: ${err.message}`)
